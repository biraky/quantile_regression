...
```
# Correlation Matrix
## Code to generate random correlation matrix ~ uniform
```{r}
...
## Code to generate fixed correlation matrix
Increasing order of correlations, jump in increase from 6:10
```{r, warning=FALSE}
...
```
# Test Fits
## Test Fits gwqs, all correlated are in model
```{r}
dataset <- generate_simulated_dataset(n_obs = 1000, 
                                      n_covariates = 15,
...
fit_gwqs <- gwqs(as.formula(form),
                  ...
                  q = 4, validation = 0.6, b = 100, rh = 20,
                  family = "gaussian", seed = 12345)

summary(fit_gwqs)
gwqs_barplot(fit_gwqs)
```
## Test Fits gwqs, with true weigths, 5 correlated are not in the model
```{r, eval = FALSE}
...
fit_gwqs <- gwqs(as.formula(form),
                  mix_name = feature_names, data = as.data.frame(dataset),
                  q = 4, validation = 0.6, b = 20, rh = 10,
                  family = "gaussian", seed = 12345)

summary(fit_gwqs)
gwqs_barplot(fit_gwqs)
```
# Functions to fit and save post processing results
## Fit wqs once 
```{r, fit and save results gwqs, eval = FALSE}
fit_wqs_once <- \(
  dataset,
  feature_names,
  ...
  form <- as.formula(
    paste0("dependent_variable ~ wqs + ", paste(biomarker_names, collapse = " + "))
  )


  fit <- gwqs(as.formula(form),
                  mix_name = feature_names, data = as.data.frame(dataset),
  ...
  rmse <- NA_real_
    ...
    rmse  <- sqrt(mean((y - unname(predict(fit)))^2))
  ...
  list(
    ...
    estimates           = list(
      ...
      final_weights_table = fit$final_weights,  # weights and CIs
      correctly_removed_proportion = sum(fit$final_weights[,2][6:10]<1/10)/5,
...
## Fit adaptive lasso once
```{r, eval = FALSE}
fit_adapt_lasso_once <- \(
  dataset,
  beta_intercept_true = 0,
  beta_true = c(rep(1, 5), rep(0, 5), rep(1, 5))
) {
...
fit_wqs_once(dataset = dataset, feature_names = feature_names)
fit_lasso_once(dataset = dataset)
fit_adapt_lasso_once(dataset = dataset)
```
# Code for simulations on server
## Wrapper
```{r, eval = FALSE}
...
collect_results <- \(dataset, feature_names) {
  ...
  grab <- \(obj, submodel, model_name) {
    tibble(
      model   = model_name,
      bias_intercept = 100 * obj[[submodel]]$bias$intercept,          # % bias intercept
      bias_biomarkers = mean(100 * unlist(obj[[submodel]]$bias$predictors[
        grepl("biomarker", names(obj[[submodel]]$bias$predictors))
      ])),                                                            # avg % bias biomarkers
  ...
  grab_wqs <- \(obj, model_name) {
    tibble(
      ...
      bias_biomarkers = mean(100 * unlist(obj$bias$biomarkers)),
...
# Post processing
## Initial look at simulation results
```{r}
load("./Simulation_results/model_results_and_datasets(2).RData")
